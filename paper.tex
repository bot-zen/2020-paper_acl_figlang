...
Attributions.
In the event that Researcher produces a written work that incorporates the
Results or limited excerpts of the Data (with prior written consent from ETS),
Researcher agrees to acknowledge the Data as property of ETS by displaying the
following attributions:  

Source: 
Derived from data provided by ETS. Copyright \textcopyright\ 2020 ETS,
\url{www.ets.org}.

Disclaimer.
Researcher must also include the following disclaimer language in all publications:  
The opinions set forth in this publication are those of the author(s) and not ETS.
